...
int menu(){
    int choice;
	printf("请选择以下一项：\n");
    printf("1.基于线性搜索的关系选择(S.C=50的元组)\n");
	printf("2.两阶段多路归并排序算法（TPMMS）,将关系R和S分别排序\n");
	printf("3.基于索引的关系选择：建立索引文件，选出S.C=50的元组\n");
	printf("4.基于排序的连接操作：对关系R和S计算S.C连接R.A\n");
	printf("5.基于排序的集合的交算法\n");
	printf("附加题：6.基于排序的集合的并算法\n");
	printf("附加题：7.基于排序的集合的差算法R-S\n");
	printf("0.退出\n");
	printf("请输入选项：");
	scanf("%d",&choice);
	return choice;
...
int func1(Buffer *buf){
	...
	init_data_blk(&res);
	int count = 0;	//记录S.C=50有多少个元组
	int cur_blk = 17;
	//********************************
	for(cur_blk = 17; cur_blk<=48; cur_blk++){
...
int setindex(Buffer *buf){
	...
	for (i=317; i<349; i++){
		...
		for (j=0; j<7; j++){
			if (i<indexnum[data.X[j]]){
				indexnum[data.X[j]] = i;//indexnum store the blk number of the first appearrance of data.X
			}
		}
		freeBlockInBuffer(blk, buf);
	}
	...
	for (i=40; i<81; i++){
		if(indexnum[i]!=999){
			...
			if(count==7){//this blk is full,but the final blk isn't sure to be full
                endpos=i;//endpos mark the final place of the full blk
				index.addr = addr+1;
	...
	//printf("endpos=%d\n",endpos);
	for(int count=0,i=endpos+1;i<81;i++){ //上面count=7是能够写满一个块的，这里用来处理剩下的不足写满一个块的索引（不能用count=7判断的）
        //printf("i=%d\n",i);
        //printf("indexnum[%d]=%d,indexnum[%d]=%d\n",i,indexnum[i],i+1,indexnum[i+1]);
        if(indexnum[i]!=999 && indexnum[i+1]!=999){
            index.X[count] = i;
			index.Y[count] = indexnum[i];
...
int func3(Buffer *buf){
	...
	for(i=pos[0]; i<pos[1]+1; i++){
		...
		for (j=0; j<7; j++){
			...
			if(data2.X[j]==50){
			    //printf("****************");
				out.X[count] = data2.X[j];
				out.Y[count] = data2.Y[j];
				count=count+1;
				printf("(X=%d, Y=%d)\n", data2.X[j], data2.Y[j]);
			}
		}
...
int func4(Buffer *buf){
	printf("------------------------------------------------------------\n");
    printf("4.基于排序的连接操作：对关系R和S计算R.A连接S.C\n");
    printf("------------------------------------------------------------\n");
